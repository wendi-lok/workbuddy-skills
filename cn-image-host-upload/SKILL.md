...
- **匿名访客每日 10 张**，超了返回 **HTTP 429** +：
  `图片上传接口限流中，每日限制10次…登录后携带 API Key 调用不受此限`
  （响应体里还有 `reset` 字段给出额度重置时间，可直接透给用户）
...
这是最容易漏的一点：**降级链成功兜住了，反而让人以为一切正常。**

真实案例：默认图床（免注册、国内直连）匿名额度 10 张/天用完，返回 429 → 自动换到了海外的
uguu，上传**成功**、返回了可用的公网 URL。但 uguu 的链接**约 3 小时后就失效**，
而下游 API 是几十分钟内随时可能来抓图。如果这时实现把失败原因丢掉了，用户只会看到"上传成功"，
然后在生成失败时完全摸不着头脑。

所以：**给会失效/降级的图床标一个 `expire` 之类的提示字段，并且在"成功但降级过"的分支里
也要返回 warning。** 别只在全部失败时才吭声。

```python
if url:                                   # 成功分支
    if errors:                            # 但前面有图床失败过 → 说明降级了
        resp["warning"] = "默认图床不可用，已自动换用「%s」。%s" % (name, expire_text)
        resp["errors"] = errors
    elif host_meta.get("expire"):         # 没降级，但这个图床本身就限时
        resp["warning"] = "当前使用「%s」。%s" % (name, expire_text)
    return resp
```

前端要优先展示这条**具体原因**，而不是笼统的"备用图床可能不稳定"。
